...
class FeatureExtractor:
    ...
    def extraer_features(self, paciente, grupo, valoracion=None, valoraciones_todas=None):
        """
        Extrae todas las features necesarias para scoring - ACTUALIZADO
        
        Args:
            paciente: Instancia de Paciente
            grupo: Instancia de GrupoTerapeutico
            valoracion: Instancia de Valoracion específica (opcional)
    ...
    def _calcular_edad_match(self, paciente, grupo):
        """
        Calcula qué tan bien la edad del paciente se ajusta al grupo - ACTUALIZADO
    ...
    def _calcular_diferencia_edad(self, paciente, grupo):
        ...
        if not asignaciones.exists():
            ...
            return abs(paciente.edad_actual - (grupo.edad_minima + grupo.edad_maxima) / 2)
        
        edad_promedio = asignaciones.aggregate(
            promedio=Avg('paciente__edad_actual')
        )['promedio']
        
        return abs(paciente.edad_actual - edad_promedio) if edad_promedio else 0
    ...
    def _calcular_tasa_exito(self, grupo):
        ...
        for asignacion in asignaciones:
            ...
            if asignacion.fecha_inicio_asignacion:
                fecha_limite = timezone.now().date() - timedelta(days=180)
                
                if asignacion.fecha_inicio_asignacion <= fecha_limite:
                    total_evaluables += 1
                    
                    # Considerar exitosa si sigue activa o duró >6 meses
                    if asignacion.estado == 'ACTIVA':
                        exitosas += 1
                    elif asignacion.estado == 'FINALIZADA' and asignacion.fecha_fin_asignacion:
                        dias = (asignacion.fecha_fin_asignacion - asignacion.fecha_inicio_asignacion).days
                        if dias >= 180:
                            exitosas += 1
    ...
    def _calcular_experiencia_grupo(self, grupo):
        ...
        """
        if not grupo.fecha_creacion:
            return 0
        
        from django.utils import timezone
        dias_activo = (timezone.now().date() - grupo.fecha_creacion.date()).days  
